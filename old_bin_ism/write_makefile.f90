subroutine output_makefile(filename)
  ...
  write(ilun,format)""
  write(ilun,format)"PATCH = ../patch/rt/frig_main"
  write(ilun,format)""
  ...
  write(ilun,format)"   NPSCAL := $(NPSCAL)+$(NIONS)+$(NDUST)+4 # for internal energy + current"
  write(ilun,format)"else"
  write(ilun,format)"   NPSCAL := $(NPSCAL)+$(NIONS)+$(NDUST)+1 # for internal energy"
  ...
  write(ilun,format)"	  -DNEXTINCT=$(NEXTINCT) -DNCR=$(NCR) -DUSE_TURB=$(USE_TURB) -DNDUST=$(NDUST) #-DQUADHILBERT=$(QUADHILBERT)"
  ...
  write(ilun,format)"VPATH = $(shell [ -z $(PATCH) ] || find $(PATCH) -type d):../$(SOLVER):../turb:../cr:../fld:../nimhd:../aton:../rt:../hydro:../pm:../poisson:../amr"
  write(ilun,format)"#############################################################################"
  ...
  write(ilun,format)"DUSTOBJ = dust_diffusion_fine.o dustdiff_split.o init_dust.o"
  write(ilun,format)"else"
  ...
  write(ilun,format)"          add_turb_forcing.o libfftw3.a"
  write(ilun,format)"endif"
  write(ilun,format)""
