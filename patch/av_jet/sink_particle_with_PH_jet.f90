...
!  if (myid .eq.1) write(*,*)' Entering d_sink ',d_sink
  ...
  rsink_max= 0. !10.*(0.5**ilevel)*scale  !(rsink_max*3.08d21/scale_l)**2
...
!           write(*,*) d,x,y,z,d_thres,d_sink

           msink_new(index_sink)=min((d-d_thres)*vol_loc,msink_max2)
           delta_mass_new(index_sink)=min((d-d_thres)*vol_loc,msink_max2)

           ! introduced by PH 09/2013 to compute the feedback from sink 
           msink_last_feed_new(index_sink)=0.
  ...
  !introduced by PH 09/2013 to compute the feedback from sink 
  call MPI_ALLREDUCE(msink_last_feed_new ,msink_last_feed_all ,nsinkmax,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
  ...
  do isink=1,nsink
     if(oksink_all(isink)==1d0)then
        ...
        !introduced by PH 09/2013 to compute the feedback from sink 
        msink_last_feed(isink)=msink_last_feed_all(isink)
...
subroutine merge_sink(ilevel)
  ...
  real(dp)::merge_max,scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2,scale_m_ms
  logical::sink_max,sink_max_loc
  

  call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)
  scale_m_ms=scale_d*scale_l**3 / 2.d33
...
!!added by PH 07/2014 prevents 2 sinks more massive than merge_max to merge
  merge_max = 3. 
...
!!added by PH 07/2014 prevents 2 sinks more massive than merge_max to merge
        ifirst=ifirst+1
        if ( msink(gndx)*scale_m_ms .gt. merge_max ) then
          sink_max = .true.
        else
          sink_max = .false.
        endif
     ...
     do while((ilast-ifirst+1)>0)
        ...
        rr=zz**2+rr
!!added by PH only sink less massive than 10 Ms can merge
        if ( msink(indx)*scale_m_ms .gt. merge_max ) sink_max_loc = .true.
!        if(myid .eq. 1) write(*,*) 'rr, rmax2 ' , rr, rmax2, .not.(sink_max .and. sink_max_loc)
        if( (rr.le.rmax2)  .and.  .not.(sink_max .and. sink_max_loc) )then
           ifirst=ifirst+1
           gsink(indx)=igrp
!!added by PH only sink less massive than 10 Ms can merge
           if (.not. sink_max) sink_max =  sink_max_loc
        else
...
!  if(myid==1)then
!     write(*,*) 'ifirst,ilast ',ifirst,ilast,sink_max,sink_max_loc
!     do isink=1,nsink
!        write(*,'(3(I3,1x),3(1PE10.3))')isink,psink(isink),gsink(isink),xsink(isink,1:ndim),msink(isink)*scale_m_ms
!     end do
!  endif

     end do
     icomp=icomp+1
!     if(myid==1)  write(*,*) 'new iteration'
  end do
  new_sink=igrp
...
!PH 29/01/2014
!a new kind of particle is introduced in order to include the jets from the sink
!these particles are located in a shell of radii between 1 and 1.5 rmax
! and have a vanishing mass thus one they have no impact on gravity
! a criteria must be added in the accretion in order to prevent them from accreting


  do kk=-2*ir_cloud,2*ir_cloud
!  do kk=-3*ir_cloud,3*ir_cloud
     zz=dble(kk)*dx_min/2.0
     do jj=-2*ir_cloud,2*ir_cloud
!     do jj=-3*ir_cloud,3*ir_cloud
        yy=dble(jj)*dx_min/2.0
        do ii=-2*ir_cloud,2*ir_cloud
!        do ii=-3*ir_cloud,3*ir_cloud
           xx=dble(ii)*dx_min/2.0
           rr=sqrt(xx*xx+yy*yy+zz*zz)
...
!                 if(rr<=rmax) mp(ind_cloud(j))=msink(isink)/dble(ncloud)
!                 if(rr>rmax .and. rr<=1.5*rmax) mp(ind_cloud(j))=0.
...
!              write(*,*) 'part ',rr/rmax,mp(ind_cloud(j))
...
subroutine grow_jeans(ilevel)
  ...
  !introduced by ph to compute the jets from the stars
!  bsink_new = 0.
     ...
     do jgrid=1,numbl(icpu,ilevel)
        ...
        if(npart1>0)then
           ...
           do jpart=1,npart1
              ...
              next_part=nextp(ipart)
!              if(idp(ipart).lt.0)then
!PH 29/01/2014 modifies this to exclude the new types of cloud particle he introduced
!which are massless
              if(idp(ipart).lt.0 .and. mp(ipart) .ne. 0.)then
                 npart2=npart2+1
              endif
              ipart=next_part  ! Go to next particle
           end do
...
!which are massless
  ...
  if(nsink>0)then
     ...
     call MPI_ALLREDUCE(msink_new,msink_all,nsinkmax     ,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
           ...
           !PH added it 29/01/2014
     call MPI_ALLREDUCE(delta_mass_new,delta_mass_all,nsinkmax,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)

     call MPI_ALLREDUCE(vsink_new,vsink_all,nsinkmax*ndim,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)

           !PH added it 29/01/2014
!     call MPI_ALLREDUCE(bsink_new,bsink_all,nsinkmax*ndim,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
...
!     write(*,*) 'delta_mass_all',delta_mass_all(isink),delta_mass_new(isink)
!     write(*,*) 'msink_all',msink_all(isink),msink_new(isink)

           !PH added it 29/01/2014
     delta_mass(isink)=delta_mass(isink)+delta_mass_all(isink)

           !PH added it 29/01/2014
           !note that "B" is calculated by adding all contribution of the particle  within the cloud
           !it is note "accreted". Also this is not normalised by the number of particles. We are interested
           !only by the direction so this is fine
!     bsink(isink,1)=bsink_all(isink,1)
!     bsink(isink,2)=bsink_all(isink,2)
!     bsink(isink,3)=bsink_all(isink,3)

     vsink(isink,1:ndim)=vsink(isink,1:ndim)/msink(isink)

!     write(*,*) 'sink, msink,delta_mass,vsink_x_y_z,bsink_x_y_z',isink,msink(isink),delta_mass(isink)
!     write(*,*) vsink(isink,1),vsink(isink,2),vsink(isink,3),bsink(isink,1),bsink(isink,2),bsink(isink,3)
...
!        bsink_new(isink,1)=bsink_new(isink,1)+(bx1+bx2)/2.
!        bsink_new(isink,2)=bsink_new(isink,2)+(by1+by2)/2.
!        bsink_new(isink,3)=bsink_new(isink,3)+(bz1+bz2)/2.
          

!        write(*,*) 'd, d_thres',d, d_thres,d_sink
        if(d>d_thres)then
...
!################################################################
!################################################################
subroutine compute_magnetic_field
  use pm_commons
  use amr_commons
  use hydro_commons
  implicit none
  ...
  ! This routine find the mean magnetic field inside the sink location
  !------------------------------------------------------------------------
  integer::ilevel,ncache,nSN,j,isink,ind,ix,iy,iz,ngrid,iskip
  integer::i,nx_loc,igrid,info
  integer,dimension(1:nvector),save::ind_grid,ind_cell
  ...
  if(nsink==0)return
  if(verbose)write(*,*)'Compute magnetic field'
...
!                    endif
                    ...
                    !we select the cells which are within the sink particle 
                    if(drr .lt. rmax2/4.)then
                       do idim=1,ndim 
                          bsink_new(isink,idim) = bsink_new(isink,idim) + vol_loc*(uold(ind_cell(i),5+idim)+uold(ind_cell(i),nvar+idim))
                       enddo
                    endif

                 end do !loop over sink
              endif
           end do
...
!  do isink=1,nsink
!           if (myid .eq. 1) write(*,*) 'bsink ', bsink(isink,1), bsink(isink,2), bsink(isink,3)
!  end do

  if(verbose)write(*,*)'Exiting compute_magnetic_field'

end subroutine compute_magnetic_field
!################################################################
!################################################################
!################################################################
!################################################################
subroutine compute_angular_momentum
  use pm_commons
  use amr_commons
  use hydro_commons
  implicit none
  ...
  if(nsink==0)return
  if(verbose)write(*,*)'Compute angular momentum'
...
!                    endif
                    ...
                    !we select the cells which are within the sink particle 
                    if(drr .lt. rmax2/4.)then
                       ang_mom_sink_new(isink,1) = ang_mom_sink_new(isink,1) + vol_loc*uold(ind_cell(i),1)*(dyy*uold(ind_cell(i),4) - dzz*uold(ind_cell(i),3))
                       ang_mom_sink_new(isink,2) = ang_mom_sink_new(isink,2) + vol_loc*uold(ind_cell(i),1)*(dzz*uold(ind_cell(i),2) - dxx*uold(ind_cell(i),4))
                       ang_mom_sink_new(isink,3) = ang_mom_sink_new(isink,3) + vol_loc*uold(ind_cell(i),1)*(dxx*uold(ind_cell(i),3) - dyy*uold(ind_cell(i),2))
                    endif
...
#ifndef WITHOUTMPI
  call MPI_ALLREDUCE(ang_mom_sink_new,ang_mom_sink_all,nsinkmax*ndim,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
!  call MPI_ALLREDUCE(vol_gas_jet,vol_gas_jet_all,nsink,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
!  call MPI_ALLREDUCE(mass_gas_agn,mass_gas_agn_all,nsink,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
!  vol_gas_jet=vol_gas_jet_all
!  mass_gas_agn=mass_gas_agn_all
  ang_mom_sink = ang_mom_sink_all
#endif
...
!  end do
  ...
  if(verbose)write(*,*)'Exiting compute_angular_momentum'
...
subroutine average_jet
  ...
  real(dp):: bx,by,bz,dxtot,cos_b_r,btot
...
!                   bz = bsink(isink,3)
                   ...
                   cos_b_r = (bx*dxx + by*dyy + bz*dzz) / dxtot


                    !we select the cells which are after outside the sink particle and aligned with the magnetic field in the sink
...
!  do isink=1,nsink
!           if (myid .eq. 1) write(*,*) 'vol_gas_jet ', vol_gas_jet(isink)
!  end do
...
subroutine put_jet
  ...
  real(dp):: bx,by,bz,dxtot,btot

  real(dp)::dm_ej,frac_acc_ej,e
...
!  write(*,*) 'enter put jet'
  ...
  Utot = 1.6d6 ! 16 km/s from Wang et al. 2010
...
!                   write(*,*) drr,rmax2,cos_b_r,dxtot,btot
                    ...
                    if(drr.lt.rmax2 .and. drr .gt. rmax2/4. .and. abs(cos_b_r) .gt. sqrt(3.)/2.)then
                     ...
                     !equal to delta_mass 
                       ...
                       coef = Utot * sqrt(msink(isink)*scale_m_ms) * cos_b_r / abs(cos_b_r)

                       u = uold(ind_cell(i),2) / d                 
                       v = uold(ind_cell(i),3) / d                
                       w = uold(ind_cell(i),4) / d                

                       u_jet = d/d_jet * u + (1.-d/d_jet) * (coef * bx + vsink(isink,1) )
                       v_jet = d/d_jet * v + (1.-d/d_jet) * (coef * by + vsink(isink,2) ) 
                       w_jet = d/d_jet * w + (1.-d/d_jet) * (coef * bz + vsink(isink,3) ) 

                       mom_tot(isink,1) = mom_tot(isink,1) + dm_ej * coef * bx
                       mom_tot(isink,2) = mom_tot(isink,2) + dm_ej * coef * by
                       mom_tot(isink,3) = mom_tot(isink,3) + dm_ej * coef * bz

!                       if(first_loc) then 
!                         write(*,*) 'vol_gas_jet ',vol_gas_jet(isink)
!                         write(*,*) 'coef ',coef,'d ',d, 'd_jet ', d_jet, 'msink ',msink(isink)*scale_m_ms
!                         write(*,*) 'delta_mass', delta_mass(isink),'u ',u, 'u_jet ',u_jet,'v ',v, 'v_jet ',v_jet
!                         first_loc = .false.
!                       endif
...
!                       bz2 = uold(ind_cell(i),nvar+3)
                       ...
                       ! Convert back to conservative variable
!#ifdef SOLVERmhd
!                       e=e+0.125d0*((bx1+bx2)**2+(by1+by2)**2+(bz1+bz2)**2)/d_jet
!#endif


                       e = uold(ind_cell(i),5) / d - 0.5d0*(u**2+v**2+w**2)
                       e = e + 0.5d0*(u_jet**2+v_jet**2+w_jet**2)

                       uold(ind_cell(i),1) = d_jet
                       uold(ind_cell(i),2) = d_jet*u_jet
                       uold(ind_cell(i),3) = d_jet*v_jet
                       uold(ind_cell(i),4) = d_jet*w_jet
                       uold(ind_cell(i),5) = d_jet*e
!                       do ivar=imetal,nvar
