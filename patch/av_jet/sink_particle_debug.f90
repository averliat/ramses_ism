...
subroutine accrete_sink(ind_grid,ind_part,ind_grid_part,ng,np,ilevel,on_creation)
  ...
  real(dp)::fbk_ener_AGN,fbk_mom_AGN,fbk_mass_AGN
  logical,dimension(1:ndim)::period
           ...
           if( .not. on_creation)then
              if(agn)then
                 if(ok_blast_agn(isink).and.delta_mass(isink)>0.0)then
                    ...
                    !---------------------------------
                    if(feedback_scheme=='mass_and_momentum')then
                       write(*,'("===========================================")')
                       write(*,'("===========================================")')
                       write(*,*)'Computing jet feedback from sink number ',isink
                       write(*,'("-----------------------")')
                       write(*,*)'delta_mass(isink) = ',delta_mass(isink)
                       fbk_mass_AGN=delta_mass(isink)/3.0d0
                       write(*,'("-----------------------")')
                       write(*,*)'fbk_mass_AGN = ',fbk_mass_AGN
                       fbk_mom_AGN=min(delta_mass(isink)*v_AGN*(180./cone_opening)*1.e5/scale_v*weight/volume*d/density,v_max*1.e5/scale_v*weight*d)
                       write(*,'("-----------------------")')
                       write(*,*)'fbk_mom_AGN = ',fbk_mom_AGN

                       ! checking if particle is in cone
                       cone_dir(1:3)=lsink(isink,1:3)/sqrt(sum(lsink(isink,1:3)**2))
                       write(*,'("-----------------------")')
                       write(*,*)'cone_dir(1:3) = ',cone_dir(1:3)
                       cone_dist=sum(r_rel(1:3)*cone_dir(1:3))
                       write(*,'("-----------------------")')
                       write(*,*)'cone_dist = ',cone_dist
                       orth_dist=sqrt(sum((r_rel(1:3)-cone_dist*cone_dir(1:3))**2))
                       write(*,'("-----------------------")')
                       write(*,*)'orth_dist = ',orth_dist
                       if (orth_dist.le.abs(cone_dist)*tan_theta)then
                          unew(indp(j,ind),2:4)=unew(indp(j,ind),2:4)+fbk_mom_AGN*r_rel(1:3)/(ir_cloud*dx_min)/vol_loc
                       write(*,'("-----------------------")')
                       write(*,*)'unew(indp(j,ind),2:4) = ',unew(indp(j,ind),2:4)
                          unew(indp(j,ind),5)=unew(indp(j,ind),5)+sum(fbk_mom_AGN*r_rel(1:3)/(ir_cloud*dx_min)*vv(1:3))/vol_loc
                       write(*,'("-----------------------")')
                       write(*,*)'unew(indp(j,ind),5) = ',unew(indp(j,ind),5)
                          unew(indp(j,ind),1)=unew(indp(j,ind),1)+fbk_mass_AGN/vol_loc
                       write(*,'("-----------------------")')
                       write(*,*)'unew(indp(j,ind),1) = ',unew(indp(j,ind),1)
                       end if

                       delta_mass_new(isink)=delta_mass_new(isink)-fbk_mass_AGN
                       write(*,'("-----------------------")')
                       write(*,*)'delta_mass_new(isink) = ',delta_mass_new(isink)
                       write(*,'("===========================================")')
                       write(*,'("===========================================")')
                    endif
     ...
     if (density>0)then
        ...
        if (dMsink_overdt(isink)>0.0)then
           ...
           if(agn)then
              ...
              !6 lines commented by AV on 02/04/19 to force jet feedback
