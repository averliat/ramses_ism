...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp), save:: max_tab_pert_aleat, var_tab_pert_aleat, mean_tab_pert_aleat, std_tab_pert_aleat, q_idl_mean, pourcent_pert, q_idl_mean_norm
  ...
  if(bb_test)then
     ...
     DO i=1,nn
       ...
       if(Mach .eq. 0)then
             ...
             !ind_i = int((x(i,1)/boxlen)*n_size)+1  !MODIF BY PHENNEBE
             !AND AVERLIAT ON 14/12/2018 FOR BIGBOX PERTURBATIONS
             !ind_j = int((x(i,2)/boxlen)*n_size)+1
             !ind_k = int((x(i,3)/boxlen)*n_size)+1
...
subroutine calc_boxlen
  ...
  logical,save:: first=.true.
    ...
    if(bb_test)then
       ...
       boxlen = r_0 * r0_box * 4. !*2. modif by averliat to have bigbox
       ...
       boxlen = r_0 * zeta * max(rap,1.) * 16. !*8.=bigbox,  *4=normalbox
